...
YOUTUBE_CHANNEL_ID_FIX = "UC6QbwXMgPiP7tenO0N6GvuA" # ID fix, evită căutarea nesigură prin sORA_START_CHECK = 15
ORA_END_CHECK = 23
INTERVAL_MINUTE = 5
...
MESAJ_VERIFICARE_METIN2 = (
 "## Verificare Metin2\n\n"
 "Ești jucător de Metin2? Apasă butonul de mai jos pentru a obține accesul la canalele de  "**Vei primi rolul:** ` Metin2`"
)
MESAJ_VERIFICARE_GAMER = (
 "## Verificare Gaming\n\n"
 "Joci alte jocuri? Apasă butonul de mai jos pentru a obține accesul la canalele de gaming "**Vei primi rolul:** ` Gamer`"
)
...
REGULAMENT_TEXT = (
 ...
 "**§4 — Conținut NSFW**\n"
 "Orice conținut pentru adulți, violent sau șocant este strict interzis și duce la ban per "**§5 — Publicitate**\n"
 "Promovarea altor servere de Discord sau site-uri externe fără acordul administrației est "**§6 — Identitate și conturi**\n"
 "Un singur cont per persoană. Conturile alternative folosite pentru a evita sancțiuni "
 "vor fi banat permanent.\n\n"
 "**§7 — Sancțiuni**\n"
 "Abaterile se sancționează progresiv: avertisment → mute → kick → ban temporar → ban perm "în funcție de gravitate.\n\n"
 "**§8 — Deciziile administrației**\n"
 "Deciziile Admin și Moderator sunt finale. Contestațiile se fac prin DM către Admin, nu p "**§9 — RMT (Real Money Trading)**\n"
 "Vânzarea, cumpărarea sau schimbul de bunuri, iteme sau conturi contra bani reali este st "interzisă pe acest server. Orice tentativă de RMT duce la ban permanent fără avertisment)
...
TEPARI_TITLU = " Raportare Țepari"
TEPARI_TEXT = (
 "Acest canal este destinat raportării tentativelor de înșelăciune sau a tranzacțiilor fra "Postează dovezile pe care le ai — screenshot-uri, nume jucător, server, descrierea situa
...
giveaway_data = {}
giveaway_history = {}
# istoric giveaway-uri finalizate recent — permite reroll. { msg_id: {premiu, participanti, cinvite_tracker = {}
cached_invites = {}
...
async def init_db():
 global db_pool
 if not DATABASE_URL:
 print(" DATABASE_URL lipsă — persistența nu va funcționa!")
 return
...
async def db_get(key: str) -> dict:
 ...
 if row:
 return json.loads(row["value"]) if isinstance(row["value"], str) else row["value" return {}
...
async def save_giveaway_data():
 """Salvează giveaway_data în PostgreSQL, convertind set/datetime în formate serializabile serializable = {}
...
async def trimite_odata(canal, titlu, text, culoare=0xE8B84B, imagine=None):
 ...
 print(f" Mesaj deja existent în #{canal.name}, skip.")
 return
...
def embed_giveaway(data: dict, guild: discord.Guild) -> discord.Embed:
 conditii_text = data.get("conditii", "").strip() or "Nicio condiție — participare liberă! membri_minim = data.get("membri_minim")
...
async def verifica_live():
 ...
 live_anuntat = False
 print(" Live încheiat, resetat flag.")
# ──────────────────────────────────────────────
...
async def verifica_membrii_giveaway():
 ...
 data["pornit"] = True
 data["end_time"] = datetime.utcnow() + timedelta(seconds=data["secunde_durata asyncio.create_task(countdown_giveaway(msg_id, data["secunde_durata"]))
...
async def update_member_counter():
 ...
 print(f" Eroare update counter membri: {e}")
# ──────────────────────────────────────────────
...
class NicknameMetin2Modal(discord.ui.Modal, title="Verificare Metin2"):
 ...
 if not rol_metin2:
 await interaction.response.send_message(" Rolul Metin2 nu a fost găsit. Contact return
 nume_rol_server = SERVERE_METIN2.get(self.server_ales)
 rol_server = discord.utils.get(guild.roles, name=nume_rol_server) if nume_rol_server  nick_nou = f"{member.name} | {self.nickname.value.strip()}"
 if len(nick_nou) > 32:
 ...
 await interaction.response.send_message(" Nu am permisiuni suficiente.", epheme return
 await verifica_si_da_verified(member, guild)
 await interaction.response.send_message(" Verificare Metin2 completată!", ephemeral # DM doar la prima obținere a rolului Metin2
 if primul_metin2:
 ...
 f" **Nickname ingame:** {self.nickname.value.strip()}\n"
 f" **Roluri primite:** {roluri_text}\n\n"
 f"Acum ai acces la canalele de Metin2. Ne vedem pe câmpul de luptă!\n\n"
 f"Mulțumim că te-ai alăturat comunității! Pentru orice problemă contactea ),
 color=0xE8B84B
 )
 embed.set_footer(text="Metin2 Community • Hydra Prestige")
 try:
...
class ServerMetin2ToggleDropdown(discord.ui.Select):
 ...
 await interaction.response.send_message(" Rolurile de server au fost actualizate!",class VerificareMetin2View(discord.ui.View):
 def __init__(self):
 super().__init__(timeout=None)
 @discord.ui.button(label="Verificare Metin2", style=discord.ButtonStyle.primary, emoji="  async def verificare_button(self, interaction: discord.Interaction, button: discord.ui.Bu rol = discord.utils.get(interaction.guild.roles, name=ROL_METIN2)
 ...
 "## Gestionează serverele Metin2\n\n = ai rolul | = nu ai rolul\nSel view=view, ephemeral=True
 )
 return
 await interaction.response.send_message(
 "## Alege serverul Metin2\n\nPe ce server joci?",
...
class JocDropdown(discord.ui.Select):
 ...
 if not rol_gamer:
 await interaction.response.send_message(" Rolul Gamer nu a fost găsit. Contacte return
 ...
 f" **Roluri primite:** {ROL_GAMER} + {jocuri_text}\n\n"
...
class JocToggleDropdown(discord.ui.Select):
 ...
 await verifica_si_da_verified(member, guild)
 await interaction.response.send_message(" Rolurile de jocuri au fost actualizate!",
class VerificareGamerView(discord.ui.View):
 def __init__(self):
 super().__init__(timeout=None)
 @discord.ui.button(label="Verificare Gaming", style=discord.ButtonStyle.success, emoji="  async def verificare_button(self, interaction: discord.Interaction, button: discord.ui.Bu rol = discord.utils.get(interaction.guild.roles, name=ROL_GAMER)
 member_role_names = [r.name for r in interaction.user.roles]
 if rol and rol in interaction.user.roles:
 ...
 await interaction.response.send_message(
 "## Gestionează jocurile tale\n\n = ai rolul | = nu ai rolul\nSelect view=view, ephemeral=True
 )
 return
 await interaction.response.send_message(
...
class GiveawayModal(discord.ui.Modal, title="Creează Giveaway"):
 premiu = discord.ui.TextInput(label="Premiu", placeholder="ex: 1.000.000 Yang", max_lengt durata = discord.ui.TextInput(
 label="Durată",
 ...
 await interaction.response.send_message(
 " Format durată invalid. Folosește ex: `3d`, `2h`, `30m`, `1d12h`.", epheme )
 return
 if secunde_durata > 365 * 86400:
 await interaction.response.send_message(" Durata maximă e 365 de zile.", epheme return
 membri_min = None
 ...
 await interaction.response.send_message(" Numărul de membri trebuie să fie  return
 canal = bot.get_channel(CANAL_GIVEAWAY_ID)
 if not canal:
 await interaction.response.send_message(" Canalul de giveaway nu a fost găsit." return
 guild = interaction.guild
 pornit = True
 ...
 pass
 await interaction.response.send_message(" Giveaway creat cu succes!", ephemeral=Tru if pornit:
...
class GiveawayView(discord.ui.View):
 def __init__(self):
 super().__init__(timeout=None)
 @discord.ui.button(label=" Participă", style=discord.ButtonStyle.success, custom_id="gi async def participa(self, interaction: discord.Interaction, button: discord.ui.Button):
 msg_id = interaction.message.id
 if msg_id not in giveaway_data:
 await interaction.response.send_message(" Giveaway-ul nu mai este activ.", ephe return
 ...
 mesaj = f"Completează condițiile și apasă **Confirm înscriere**:\n\n{conditii}" if co await interaction.response.send_message(mesaj, view=ConfirmView(msg_id), ephemeral=Trasync def inscrie_participant_giveaway(msg_id: int, user: discord.User, guild: discord.Guild, """Înscrie un user la giveaway — folosit de butonul Confirm și de /adauga-participant.
 Returnează True dacă s-a înscris cu succes, False dacă era deja înscris sau giveaway-ul n if msg_id not in giveaway_data:
 return False
 data = giveaway_data[msg_id]
 if user.id in data["participanti"]:
 return False
 ...
 try:
 embed_dm = discord.Embed(
 title=" Linkul tău de invitație",
 description=(
 f"Ești înscris la giveaway! \n\n"
 f"**Linkul tău unic pentru a invita prieteni pe server:**\n{invite.ur f"Cu cât inviți mai mulți prieteni, cu atât giveaway-ul pornește mai  ),
 color=0xFEE75C
 )
 ...
 f" **{user.display_name}** s-a înscris la giveaway-ul **{data['premiu']}**{ f" Total participanți: **{total}**"
 )
 except Exception:
 pass
 return True
class ConfirmView(discord.ui.View):
 ...
 await interaction.response.send_message(" Giveaway-ul nu mai este activ.", ephe return
 succes = await inscrie_participant_giveaway(self.msg_id, interaction.user, interactio if not succes:
 await interaction.response.send_message(" Ești deja înscris!", ephemeral=True)
 return
...
async def finalizeaza_giveaway(msg_id: int):
 ...
 castigatori_ids = []
 participanti_ramasi = list(participanti)
 for _ in range(numar_castigatori):
 castigator_id = random.choice(participanti_ramasi)
 castigatori_ids.append(castigator_id)
 participanti_ramasi.remove(castigator_id)
 if not participanti_ramasi:
 break
 linii_castigatori = []
...
# ──────────────────────────────────────────────
cufere_data = {}
# { msg_id: { intrebare, end_time, secunde_durata, canal_id, raspunsuri: [(user_id, text, timdef embed_cufere(data: dict) -> discord.Embed:
...
class CufereRaspunsModal(discord.ui.Modal, title="Răspunsul tău"):
 ...
 await interaction.response.send_message(" Jocul nu mai este activ.", ephemeral= return
 ...
 data["raspunsuri"][existing_idx] = (user.id, self.raspuns.value.strip(), datetime await interaction.response.send_message(" Răspunsul tău a fost actualizat!", ep else:
 data["raspunsuri"].append((user.id, self.raspuns.value.strip(), datetime.utcnow() await interaction.response.send_message(" Răspunsul tău a fost înregistrat!", eclass CufereView(discord.ui.View):
 def __init__(self):
 super().__init__(timeout=None)
 @discord.ui.button(label=" Ghicește", style=discord.ButtonStyle.primary, custom_id="cuf async def ghiceste(self, interaction: discord.Interaction, button: discord.ui.Button):
 msg_id = interaction.message.id
...
class CufereModal(discord.ui.Modal, title="Creează Joc Cufere"):
 ...
 pass
 await interaction.response.send_message(" Joc Cufere creat cu succes!", ephemeral=T asyncio.create_task(finalizeaza_cufere(msg.id, secunde_durata))
...
async def on_member_join(member: discord.Member):
 ...
 print(f" Eroare invite tracking: {e}")
...
async def mute(interaction: discord.Interaction, membru: discord.Member, motiv: str = "Nespec if not are_rol_staff(interaction.user):
 await interaction.response.send_message(" Nu ai permisiunea.", ephemeral=True)
 return
 try:
 await membru.edit(mute=True)
 embed = discord.Embed(title=" Mute aplicat", description=f"**Membru:** {membru.ment embed.set_footer(text=f"Acțiune de: {interaction.user.name}")
 await interaction.response.send_message(embed=embed)
 except discord.Forbidden:
 await interaction.response.send_message(" Nu am permisiuni suficiente.", ephemeral=@tree.command(name="timeout", description="Aplică timeout unui membru")
@app_commands.describe(membru="Membrul", minute="Durata în minute", motiv="Motivul")
async def timeout_cmd(interaction: discord.Interaction, membru: discord.Member, minute: int,  if not are_rol_staff(interaction.user):
 await interaction.response.send_message(" Nu ai permisiunea.", ephemeral=True)
 return
 try:
 await membru.timeout(timedelta(minutes=minute), reason=motiv)
 embed = discord.Embed(title=" Timeout aplicat", description=f"**Membru:** {membru.m embed.set_footer(text=f"Acțiune de: {interaction.user.name}")
 await interaction.response.send_message(embed=embed)
 except discord.Forbidden:
 await interaction.response.send_message(" Nu am permisiuni suficiente.", ephemeral=@tree.command(name="kick", description="Dă afară un membru din server")
@app_commands.describe(membru="Membrul", motiv="Motivul")
async def kick(interaction: discord.Interaction, membru: discord.Member, motiv: str = "Nespec if not are_rol_staff(interaction.user):
 await interaction.response.send_message(" Nu ai permisiunea.", ephemeral=True)
 return
 try:
 await membru.kick(reason=motiv)
 embed = discord.Embed(title=" Kick aplicat", description=f"**Membru:** {membru.name embed.set_footer(text=f"Acțiune de: {interaction.user.name}")
 await interaction.response.send_message(embed=embed)
 except discord.Forbidden:
 await interaction.response.send_message(" Nu am permisiuni suficiente.", ephemeral=@tree.command(name="ban", description="Banează un membru permanent")
@app_commands.describe(membru="Membrul", motiv="Motivul")
async def ban(interaction: discord.Interaction, membru: discord.Member, motiv: str = "Nespeci if not are_rol_staff(interaction.user):
 await interaction.response.send_message(" Nu ai permisiunea.", ephemeral=True)
 return
 try:
 await membru.ban(reason=motiv)
 embed = discord.Embed(title=" Ban aplicat", description=f"**Membru:** {membru.name} embed.set_footer(text=f"Acțiune de: {interaction.user.name}")
 await interaction.response.send_message(embed=embed)
 except discord.Forbidden:
 await interaction.response.send_message(" Nu am permisiuni suficiente.", ephemeral=class AnuntModal(discord.ui.Modal, title="Creează Anunț"):
 titlu = discord.ui.TextInput(label="Titlu", placeholder="ex: Eveniment nou: Duelul Profeț text = discord.ui.TextInput(
 label="Conținut",
 ...
 if not canal:
 await interaction.response.send_message(" Canalul nu a fost găsit.", ephemeral= return
 embed = discord.Embed(
 ...
 await canal.send(embed=embed)
 await interaction.response.send_message(f" Anunț postat în <#{self.canal_id}>!", ep@tree.command(name="anunt", description="Postează un anunț formatat ca embed într-un canal al@app_commands.describe(canal="Canalul în care se postează anunțul")
...
@tree.command(name="cufere", description="Creează un joc de ghicit (ex: câte cufere dropezi)"async def cufere_cmd(interaction: discord.Interaction):
 if not are_rol_staff(interaction.user):
 await interaction.response.send_message(" Nu ai permisiunea.", ephemeral=True)
 return
...
async def help_cmd(interaction: discord.Interaction):
 ...
 "`/reroll-giveaway [message_id]` — extrage alt câștigător dacă cel curent nu înde "Right-click pe membru → Apps → **Adaugă la giveaway** — adaugă manual un partici ),
 inline=False
 )
 embed.add_field(
 name=" Cufere",
...
async def giveaway_cmd(interaction: discord.Interaction, mentioneaza_everyone: bool = True, n if not are_rol_staff(interaction.user):
 await interaction.response.send_message(" Nu ai permisiunea.", ephemeral=True)
 return
 if numar_castigatori < 1 or numar_castigatori > 20:
 await interaction.response.send_message(" Numărul de câștigători trebuie să fie înt return
 await interaction.response.send_modal(GiveawayModal(mentioneaza_everyone, numar_castigatoclass AlegeGiveawayDropdown(discord.ui.Select):
 ...
 succes = await inscrie_participant_giveaway(msg_id, self.target_user, interaction.gui if succes:
 await interaction.response.send_message(
 f" {self.target_user.mention} a fost adăugat la giveaway-ul **{giveaway_dat ephemeral=True
 )
 else:
 await interaction.response.send_message(
 f" {self.target_user.mention} este deja înscris la acest giveaway.",
 ephemeral=True
 )
@tree.context_menu(name="Adaugă la giveaway")
async def adauga_participant_context(interaction: discord.Interaction, member: discord.Member if not are_rol_staff(interaction.user):
 await interaction.response.send_message(" Nu ai permisiunea.", ephemeral=True)
 return
 if not giveaway_data:
 await interaction.response.send_message(" Nu există niciun giveaway activ momentan. return
 if len(giveaway_data) == 1:
 msg_id = list(giveaway_data.keys())[0]
 succes = await inscrie_participant_giveaway(msg_id, member, interaction.guild, adauga if succes:
 await interaction.response.send_message(
 f" {member.mention} a fost adăugat la giveaway-ul **{giveaway_data[msg_id] ephemeral=True
 )
 else:
 await interaction.response.send_message(
 f" {member.mention} este deja înscris la acest giveaway.",
 ephemeral=True
 )
 return
...
@tree.command(name="reroll-giveaway", description="Extrage alt câștigător dacă cel curent nu @app_commands.describe(message_id="ID-ul mesajului giveaway-ului încheiat", exclude="ID-ul usasync def reroll_giveaway(interaction: discord.Interaction, message_id: str, exclude: str = N if not are_rol_staff(interaction.user):
 await interaction.response.send_message(" Nu ai permisiunea.", ephemeral=True)
 return
 try:
 ...
 return
 # Pentru reroll căutăm în istoricul mesajelor, deoarece giveaway-ul e deja șters din give # Folosim un mecanism simplu: cerem staff-ul să dea reroll imediat după extragere, cât ma backup = giveaway_history.get(msg_id)
 if not backup:
 await interaction.response.send_message(
 " Nu mai există date pentru acest giveaway (s-a încheiat și nu mai e în istoric )
 return
 participanti = list(backup["participanti"])
 exclude_id = None
 if exclude:
 try:
 exclude_id = int(exclude.strip().replace("<@", "").replace(">", "").replace("!",  except ValueError:
 exclude_id = None
 if exclude_id and exclude_id in participanti:
 participanti.remove(exclude_id)
 if not participanti:
 await interaction.response.send_message(" Nu mai există participanți eligibili pent return
...
@tree.command(name="participanti-giveaway", description="Vezi câți participanți sunt la un gi@app_commands.describe(message_id="ID-ul mesajului giveaway-ului")
async def participanti_giveaway(interaction: discord.Interaction, message_id: str):
 ...
 return
 if msg_id not in giveaway_data:
 await interaction.response.send_message(" Nu există niciun giveaway activ cu acest  return
 data = giveaway_data[msg_id]
 participanti = list(data["participanti"])
 embed = discord.Embed(
 ...
 embed.set_footer(text="Hydra Prestige • Metin2 Community")
 await interaction.response.send_message(embed=embed, ephemeral=True)
@tree.command(name="extrage-giveaway", description="Extrage câștigătorul unui giveaway înaint@app_commands.describe(message_id="ID-ul mesajului giveaway-ului")
async def extrage_giveaway(interaction: discord.Interaction, message_id: str):
 ...
 canal = bot.get_channel(data["canal_id"])
 await finalizeaza_giveaway(msg_id)
 await interaction.response.send_message(" Giveaway extras cu succes, înainte de termen!
...
async def anuleaza_giveaway(interaction: discord.Interaction, message_id: str):
 ...
 description=f"Giveaway-ul pentru **{data['premiu']}** a fost anulat de {interaction.u color=0xED4245
 )
 embed.set_footer(text="Hydra Prestige • Metin2 Community")
 await canal.send(embed=embed)
 del giveaway_data[msg_id]
...
async def invitatii_cmd(interaction: discord.Interaction, membru: discord.Member = None):
 ...
 description=f"**{target.display_name}** a adus **{count}** membre pe server.{inviter_ color=0x5865F2
 )
 await interaction.response.send_message(embed=embed, ephemeral=True)
@tree.command(name="leaderboard-invitatii", description="Vezi clasamentul membrilor după număasync def leaderboard_invitatii_cmd(interaction: discord.Interaction):
 lista = await get_leaderboard_invitatii()
 if not lista:
 await interaction.response.send_message(" Nimeni nu a adus încă invitații.", epheme return
 linii = []
 medalii = [" ", " ", " "]
 ...
 embed.set_footer(text="Hydra Prestige • Metin2 Community")
 await interaction.response.send_message(embed=embed)
@tree.command(name="seteaza-invitatii", description="Setează manual numărul de invitații al u@app_commands.describe(membru="Membrul de modificat", numar="Numărul nou de invitații")
async def seteaza_invitatii_cmd(interaction: discord.Interaction, membru: discord.Member, num if not are_rol_staff(interaction.user):
 await interaction.response.send_message(" Nu ai permisiunea.", ephemeral=True)
 return
 if numar < 0:
 await interaction.response.send_message(" Numărul nu poate fi negativ.", ephemeral= return
...
async def on_ready():
 ...
 if owner:
 await owner.send(f" Botul a pornit/restartat cu succes la `{datetime.utcnow().s except Exception:
 pass
 ...
 if canal:
 await trimite_odata(canal, REGULAMENT_TITLU, REGULAMENT_TEXT, 0xE8B84B, REGULAMENT_IM canal = bot.get_channel(CANAL_FAQ_ID)
 if canal:
